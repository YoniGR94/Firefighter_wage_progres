...
```{r data usa api 2}
df_fire<- get_fire$data %>%
  select(c(2,4:7)) %>%
  rename(Average_Wage= 'Average Wage',
         'Mean Wage Appx MOE' = 'Average Wage Appx MOE') %>%
...
#לתקן פונקציה ולהתאים לטבלה 16ת18
call_fire<- function(x,year, m_TF= T)
  {
  my_get<- GET(x) %>%
    content(as= 'text', encoding = "UTF-8") %>%
    readHTMLTable(trim=T, as.data.frame=T, header=m_TF, skip.rows =  1:3)
  if(m_TF)
  {my_get<- bind_rows(my_get)[4:6]} else {my_get<-my_get[[3]]}
  my_get<- my_get %>%
...
firelist<- rbind(df_14,df_15,df_16,df_17,df_18,df_19) %>%
  mutate(across(c(fires_Num,Acres_burned), ~map_chr(x., ifelse(.x== "(1)", 0, .x))))
  #fires_Num=replace(fires_Num, fires_Num=="(1)", 0)) %>%
...
fire_full %>%
  ...
  labs(title = 'Avarage Wage by Year')+ xlab("Year") +ylab(" ")+
...
A<- fire_full %>%
  ...
  scale_x_continuous(labels = scales::comma, limits = c(0,2500000))

B<- fire_full[full_6,] %>%
  ggplot(aes(x= Acres_burned ,y= log_Wage))+geom_smooth()+
  geom_smooth(method = "lm", colour= "red")+ylim(9.75,11.2)+
  labs(title= "Acres Effect on Wage State of Big 6",caption= "red: lm model, blue: loess model")+
  xlab("Acres burned")+ scale_x_continuous(labels = scales::comma, limits = c(0,2500000))
